...
void PrecomputedTransactionData::Init(const T& txTo, std::vector<CTxOut> spent_outputs)
{
    ...
    if (g_con_elementsmode && !m_spent_outputs.empty()) {
        ...
        for (size_t i = 0; i < txTo.vin.size(); ++i) {
            ...
            simplicityRawInput[i].txo.asset = m_spent_outputs[i].nAsset.vchCommitment.data();
            simplicityRawInput[i].txo.value = m_spent_outputs[i].nValue.vchCommitment.data();
            ...
            simplicityRawInput[i].issuance.amount = txTo.vin[i].assetIssuance.nAmount.vchCommitment.data();
            simplicityRawInput[i].issuance.inflationKeys = txTo.vin[i].assetIssuance.nInflationKeys.vchCommitment.data();
        }

        std::vector<rawOutput> simplicityRawOutput(txTo.vout.size());
        for (size_t i = 0; i < txTo.vout.size(); ++i) {
            simplicityRawOutput[i].asset = txTo.vout[i].nAsset.vchCommitment.data();
            simplicityRawOutput[i].value = txTo.vout[i].nValue.vchCommitment.data();
            simplicityRawOutput[i].nonce = txTo.vout[i].nNonce.vchCommitment.data();
...
bool GenericTransactionSignatureChecker<T>::CheckSimplicity(const CScriptWitness& witness, const std::vector<unsigned char>& program, ScriptError* serror) const
{
    ...
    const uint32_t cmr[8] = { program[0] << 24 | program[1] << 16 | program[2] << 8 | program[3]  // :TODO: have elements_simplicity_execSimplicity take an array of unsigned char for CMR and WMRs
                            , program[4] << 24 | program[5] << 16 | program[6] << 8 | program[7]
                            , program[8] << 24 | program[9] << 16 | program[10] << 8 | program[11]
                            , program[12] << 24 | program[13] << 16 | program[14] << 8 | program[15]
                            , program[16] << 24 | program[17] << 16 | program[18] << 8 | program[19]
                            , program[20] << 24 | program[21] << 16 | program[22] << 8 | program[23]
                            , program[24] << 24 | program[25] << 16 | program[26] << 8 | program[27]
                            , program[28] << 24 | program[29] << 16 | program[30] << 8 | program[31]
                            };
    if (!elements_simplicity_execSimplicity(&success, txdata->simplicityTxData, nIn, cmr, 0, file)) {
